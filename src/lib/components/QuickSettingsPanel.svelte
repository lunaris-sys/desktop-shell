<script lang="ts">
  /// Quick Settings Panel (#52) with persistent config via shell.toml.
  ...
  // Brightness as 0-100 integer for the slider.
...
<style>
  ...
  .qs-panel {
    ...
    border-radius: 10px; background: var(--color-bg-shell);
    border: 1px solid color-mix(in srgb, var(--color-fg-shell) 20%, transparent);
    box-shadow: 0 8px 24px rgba(0, 0, 0, 0.4); color: var(--color-fg-shell);
  ...
  .qs-slider-thumb { position: absolute; left: var(--value); width: 14px; height: 14px; background: var(--color-fg-shell); border-radius: 50%; transform: translateX(-50%); box-shadow: 0 1px 4px rgba(0,0,0,0.3); pointer-events: none; }
  ...
  .qs-icon-btn { width: 28px; height: 28px; display: flex; align-items: center; justify-content: center; gap: 2px; background: transparent; border: 1px solid transparent; border-radius: 6px; color: color-mix(in srgb, var(--color-fg-shell) 50%, transparent); cursor: pointer; transition: all 0.15s ease; padding: 0; }
  .qs-icon-btn:hover { background: color-mix(in srgb, var(--color-fg-shell) 10%, transparent); color: var(--color-fg-shell); }

  .qs-user-row { display: flex; align-items: center; gap: 10px; padding: 10px 12px; }
  .qs-avatar { width: 32px; height: 32px; border-radius: 50%; background: color-mix(in srgb, var(--color-fg-shell) 15%, transparent); display: flex; align-items: center; justify-content: center; font-size: 0.6875rem; font-weight: 600; color: var(--color-fg-shell); flex-shrink: 0; user-select: none; }
  .qs-user-name { flex: 1; font-size: 0.8125rem; color: var(--color-fg-shell); }

  .qs-power-dropdown { position: relative; }
  .qs-power-menu { position: absolute; bottom: 100%; right: 0; margin-bottom: 4px; min-width: 140px; background: var(--color-bg-shell); border: 1px solid color-mix(in srgb, var(--color-fg-shell) 20%, transparent); border-radius: 8px; padding: 4px; box-shadow: 0 4px 16px rgba(0,0,0,0.3); z-index: 110; }
  .qs-power-menu button { width: 100%; display: flex; align-items: center; gap: 8px; padding: 8px 10px; background: transparent; border: none; border-radius: 6px; color: var(--color-fg-shell); font-size: 0.75rem; cursor: pointer; text-align: left; transition: background-color 0.1s ease; }
